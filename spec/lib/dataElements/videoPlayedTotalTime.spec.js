...
describe('videoPlayedTotalTime data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/videoPlayedTotalTime');
  const getBaseEvent = require('../../specHelpers/getBaseEvent');

  beforeEach(() => {
    this.event = getBaseEvent(['videoPlayedTotalTime']);
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "videoPlayedTotalTime" property is missing',
      () => {
        delete this.event.vimeo.videoPlayedTotalTime;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
