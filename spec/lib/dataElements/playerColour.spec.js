...
describe('playerColour data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/playerColour');
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "playerColour" property is missing',
      () => {
        delete this.event.vimeo.playerColour;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
