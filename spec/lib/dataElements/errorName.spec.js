...
describe('errorName data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/errorName');
  const getBaseEvent = require('../../specHelpers/getBaseEvent');

  beforeEach(() => {
    this.event = getBaseEvent(['errorName']);
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "errorName" property is missing',
      () => {
        delete this.event.vimeo.errorName;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
