...
describe('playerAutopaused data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/playerAutopaused');
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "playerAutopaused" property is missing',
      () => {
        delete this.event.vimeo.playerAutopaused;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
