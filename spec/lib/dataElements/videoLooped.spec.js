...
describe('videoLooped data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/videoLooped');
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "videoLooped" property is missing',
      () => {
        delete this.event.vimeo.videoLooped;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
