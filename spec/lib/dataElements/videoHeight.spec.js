...
describe('videoHeight data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/videoHeight');
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "videoHeight" property is missing',
      () => {
        delete this.event.vimeo.videoHeight;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
