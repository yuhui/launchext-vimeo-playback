...
describe('videoMuted data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/videoMuted');
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "videoMuted" property is missing',
      () => {
        delete this.event.vimeo.videoMuted;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
