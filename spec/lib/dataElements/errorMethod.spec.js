...
describe('errorMethod data element delegate', () => {
  const dataElementDelegate = require('../../../src/lib/dataElements/errorMethod');
  const getBaseEvent = require('../../specHelpers/getBaseEvent');

  beforeEach(() => {
    this.event = getBaseEvent(['errorMethod']);
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "errorMethod" property is missing',
      () => {
        delete this.event.vimeo.errorMethod;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
