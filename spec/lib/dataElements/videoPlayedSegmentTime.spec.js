...
describe('videoPlayedSegmentTime data element delegate', () => {
  ...
  beforeEach(() => {
    this.event = getBaseEvent(['videoPlayedSegmentTime']);
  ...
  describe('with invalid "event" argument', () => {
    ...
    it(
      'should be undefined when "videoPlayedSegmentTime" property is missing',
      () => {
        delete this.event.vimeo.videoPlayedSegmentTime;
        const result = dataElementDelegate(this.settings, this.event);
        expect(result).toBeUndefined();
      }
    );
